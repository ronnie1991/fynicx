     ...
     <input type="hidden" name="form_id" value="<?= $_SESSION["formid"]?>" />
              <div class="row">
                 ...
                 <div class="form-group">
                    ...
                    <select name="emp_codeNm" class="form-control select2" style="width: 100%;" required="required">
                      <option value="">Select A Employee </option>
                       <?php foreach($object->allEmployee() as $row) {
                        $chkStatus=$object->singelEmpAdmLognDetils($row['emp_code']);                       
                        if($chkStatus==0)
                        {
                        ?>
                      <option value="<?= $row['emp_code'];?>"><?= $row['emp_code'];?> / <?= $row['emp_name'];?></option>  
                      <?php } } ?>            
                    </select>
                  </div><!-- /.form-group -->                
                </div><!-- /.col --> 
                ...
                <div class="form-group">
                  ...
                  <input id="lognUsrId" type="text" name="emp_userid" class="form-control" pattern="[a-zA-Z][a-zA-Z\s]*" minlength="5" maxlength="50" oninvalid="setCustomValidity('Enter Valid User Id (Minimum 5 Characters)')"  onchange="try{setCustomValidity('')}catch(e){}" placeholder="Enter a User Id (minimum 5 charecters)"required>
                </div>                 
              </div><!-- /.col -->                   
      </div><!-- /.row -->
        <div class="row">
        ...
        <input type="text" id="password" name="emp_confirm_password" pattern=".{6,}" oninvalid="setCustomValidity('Enter Minimum 6 Characters ')"  onchange="try{setCustomValidity('')}catch(e){}" class="form-control" placeholder="Enter a password (minimum 6 charecters)"required>
        </div>                 
        </div><!-- /.col -->     
        <div class="col-md-6">                
        <div class="form-group">
        ...
        <div class="col-sm-6">
        <div class="form-group">         
        <button id="submt" type="submit" name="add_class_sec" class="btn btn-primary">Submit</button>
        </div>                 
        </div><!-- /.col -->         
        </div><!-- /.row -->                
        </form>
