			 ...
			 <input type="hidden" name="form_id" value="<?= $_SESSION['session_form'];?>" />
                <div class="col-md-3">
                  <div class="form-group">
                    <label>Upgraded Year</label>
                    <select id="yerId"  class="form-control select2 admyear" name="year" required style="width: 100%;">
                      <option value="" >Select Year</option>
                       <?php 
					      $fromYr=base64_decode($_GET['admyr']);
					   for($i=2000;$i<=2025;$i++) { ?>
                      <option <?php if($i==$fromYr){ echo 'disabled=disabled'; } ?> value="<?= $i;?>" >
					  <?= $i;?></option>
                        <?php } ?>
                    </select>                  					
                  </div><!-- /.form-group -->                  
				...
				<div class="col-md-3 cls" style="display:none">
                  <div class="form-group">
                    <label>Upgraded In Class</label>
                    <select id="classId" class="form-control select2 st_class" name="class" required style="width: 100%;">
                      <option value="" >Select a class</option>
                       <?php
                            $fromYr=base64_decode($_GET['admcls']);					   
					   foreach($object->findallclass() as $row) { ?>
                      <option <?php if($row['id']==$fromYr) { echo 'disabled=disabled'; } ?> value="<?= $row['id'];?>" >
					  <?= $row['class'];?></option>
                        <?php } ?>
                    </select>                  					
                  </div><!-- /.form-group -->                  
		  ...
		  $(".admyear").change(function(){            
			  var rwYrId=$(this).val();
                 ...
                 if(yar!='')
				 {
					 ...
					 var modif_url=curnt_url+'&upgyr='+yar;					 
					 history.pushState({},null,modif_url);					
				 }					 
			        	
		});  
		  $(document).on('change','.st_class',function(){
			     var rwid=$(this).val();
			   var clsid=btoa(rwid); 
			   	var curnt_url=window.location.href;  
                 var modif_url=curnt_url+'&upgcls='+clsid;					 
			     history.pushState({},null,modif_url);
                  var parms=window.location.href.split('?');                   			  
			    window.location.href = 'studupgrd_list?'+parms[1]; 			  
		  });
