	  ...
	  <table id="example1" class="table table-bordered table-striped">
		...
		<tbody>  
		  <?php		 
			 ...
			 $upgdSec=base64_decode($_GET['aupgsec']);		  
		     ...
		     $chkUpStCsy=$object->chkUpgradeStuCsyr($stRo['admission_number'],$upgdyar,$upgdClas,$upgdSec);			 
    ...
    <script>
      $(function () {
		  $(document).on('click','.upnwrol',function(){		
			...
			var upsec =" <?php echo $_GET['aupgsec']; ?>";
			...
			if(!isNaN(trmRolNo)) {  
             $.ajax({
                  ...
                  if(data.indexOf("Roll Number / Available") > -1)
				  {
					...
					cache:false,
					success:function(data){
                    var colIn=costr.find("td:nth-child(4)").html(trmRolNo);   						
                    var colIn=costr.find("td:nth-child(5)").remove();   						
					console.log(colIn);
					} 
					});
				  }	
