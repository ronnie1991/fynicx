  ...
  <body class="hold-transition skin-blue sidebar-mini">
     ...
     <?php include_once('left_asid.php');?>
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
           Admission charges for the new student
            <small>Add charges</small>
          </h1>
          <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
            <li><a href="#">General Admission Charges</a></li>
            <li class="active">Add General Admission Charges</li>
          </ol>
        </section>
        ...
        <section class="content">
          ...
          <!-- SELECT2 EXAMPLE -->
		  <?php
				if(isset($_POST['receive_genadm_charges']))
				{
				if($_POST['form_id']==$_SESSION['session_form'])
				{
				$_SESSION['session_form']='';
				$msg=$object->receveGenAdminCharg();
				}                					
				}
				else
				{
			...
			<form method="post">
			 <input type="hidden" name="form_id" value="<?= $_SESSION['session_form'];?>" />
				...
				<div class="col-sm-3">
				   <div class="form-group">              
                    <label>
                      <input type="radio" name="genadmch_sta" class="flat-red" value="1" required >
                      Enable Payment
                    </label>
                  </div>
                  <!-- /.form-group -->                  
				...
				<div class="col-sm-3">
				   <div class="form-group">            
                    <label>
                      <input type="radio" name="genadmch_sta" value="0" class="flat-red" required>
                     Dissabled Payment
                    </label>
                  </div>
                  <!-- /.form-group -->                  
				 ...
				 <div class="col-md-6">
                   <div class="box-footer">
                    <button type="submit" name="receive_genadm_charges" class="btn btn-block btn-primary btn-flat">Submit</button>
                  </div>                 
                </div><!-- /.col -->
				</form>
              </div><!-- /.row -->
    ...
    <script>
      $(function () {
		...
		$.ajax({
				  url:'ajax_gen_admission_charges',
				  data:{id:id},
				  type : 'POST' ,
				  cache:false,
				  success:function(data){
		...
		$.ajax({
				  url:'ajax_recv_genad_chrgs',
				  data:{id:id},
				  type : 'POST' ,
				  cache:false,
				  success:function(data){
				  $(".recv_genadm_chrgs").html(data);			
				 } 
		});
		});  
		 $(document).on('change','.sec',function(){
			   ...
			   var year=btoa(rwYear);			   
			    $.ajax({
				  url:'ajax_roll_num_generl_admsn',
