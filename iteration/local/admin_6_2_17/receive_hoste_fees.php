  ...
  <body class="hold-transition skin-blue sidebar-mini">
     ...
     <?php include_once('left_asid.php');?>
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
          Receive Hostel Fees From Student
            <small>Receive Hostel Fees</small>
          </h1>
          <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
            <li><a href="#">Hostel Fees</a></li>
            <li class="active">Receive Hostel Fees From Student</li>
          </ol>
        </section>
        ...
        <section class="content">
		  ...
		  <?php
				...
				if(isset($_POST['receive_hostl_fees']))
				{
				if($_POST['form_id']==$_SESSION['session_form'])
				{
				$_SESSION['session_form']='';
				$msg=$object->receveHostlFees();
				}                					
				}
				else
				{
			 ...
			 <input type="hidden" name="form_id" value="<?= $_SESSION['session_form'];?>" />
                  ...
                  <div class="form-group">
                    ...
                    <select id="month" class="form-control select2" name="month" required style="width: 100%;">
                      <option value="" >Select Month</option>
                       <?php for($i=1;$i<=12;$i++) { ?>
                      <option <?php if(date("m")==$i){echo 'selected=selected';} ?> value="<?= $i;?>" ><?= $i;?></option>
                        <?php } ?>
                    </select>                  					
                  </div><!-- /.form-group -->                  
                </div><!-- /.col -->	
				...
				<div class="col-md-6">
                  <div class="form-group">
                    ...
                    <select class="form-control select2 fesscateg" name="fee_category" required style="width: 100%;">
                      <option value="" >Select a fees category</option>                      
                      <option value="1" >General</option>                      
                      <option value="2" >Villagers</option>                      
                      <option value="3" >ST </option>                      
                    </select>                  					
                  </div><!-- /.form-group -->                  
                </div><!-- /.col -->
				<div class="st_hostel_fee">
                </div>
				<div class="st_roll_nm">
                </div>
                     				
				<div class="recv_host_fes">
                                  
                </div><!-- /.col -->			
				<div class="col-sm-3">
				   <div class="form-group">              
                    <label>
                      <input type="radio" name="hosfee_stat" class="flat-red" value="1" required >
                      Enable Payment
                    </label>
                  </div>
                  <!-- /.form-group -->                  
				...
				<div class="col-sm-3">
				   <div class="form-group">            
                    <label>
                      <input type="radio" name="hosfee_stat" value="0" class="flat-red" required>
                     Dissabled Payment
                    </label>
                  </div>
                  <!-- /.form-group -->                  
				 ...
				 <div class="col-md-6">
                   <div class="box-footer">
                    <button type="submit" name="receive_hostl_fees" class="btn btn-block btn-primary btn-flat">Submit</button>
                  </div>                 
                </div><!-- /.col -->
				</form>
              </div><!-- /.row -->
    ...
    <script>
      $(function () {
		 ...
		 $(document).on('change','.fesscateg',function(){
			    var rwid=$(this).val();
			   var id=btoa(rwid); 
			   var rwclid=$("#classId").val();			  
			   var clid=btoa(rwclid);			  
			    $.ajax({
				  url:'ajax_hostel_fees',
				  data:{id:id,cls:clid},
				  type : 'POST' ,
				  cache:false,
				  success:function(data){
				  $(".st_hostel_fee").html(data);			
				 } 
		      }); 
             $.ajax({
				  url:'ajax_recvhos_fess',
				  data:{id:id,clid:clid},
				  type : 'POST' ,
				  cache:false,
				  success:function(data){
				  $(".recv_host_fes").html(data);			
				 } 
		       });			  
		  });
		  $(document).on('change','.sec',function(){
			   ...
			   var year=btoa(rwYar);
			   var rwMnt=$("#month").val();			  
			   var month=btoa(rwMnt);			  
			    $.ajax({
				  url:'ajax_roll_num_recv_hosfee',
