  ...
  <body class="hold-transition skin-blue sidebar-mini">
     ...
     <?php include_once('left_asid.php');?>
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
          Receive Other Annual Charges From Student
            
          </h1>
          <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
            <li><a href="#">Other Annual Charges</a></li>
            <li class="active">Receive Other Annual Charges From Student</li>
          </ol>
        </section>
        ...
        <section class="content">
		  ...
		  <?php
				...
				if(isset($_POST['receive_otheran_charges']))
				{
				if($_POST['form_id']==$_SESSION['session_form'])
				{
				$_SESSION['session_form']='';
				$msg=$object->receveOtherAnualChrgs();
				}                					
				}
				else
				{
          ...
          <div class="box box-default">
			...
			<form method="post">
			 <input type="hidden" name="form_id" value="<?= $_SESSION['session_form'];?>" />
				...
				<div class="st_otan_chr">
                </div>
				<div class="st_roll_nm">
                </div>
                     				
				<div class="recv_otan_chrgs">
                                    
                </div><!-- /.col -->			
				<div class="col-sm-3">
				   <div class="form-group">              
                    <label>
                      <input type="radio" name="otanch_stat" class="flat-red" value="1" required >
                      Enable Payment
                    </label>
                  </div>
                  <!-- /.form-group -->                  
				...
				<div class="col-sm-3">
				   <div class="form-group">            
                    <label>
                      <input type="radio" name="otanch_stat" value="0" class="flat-red" required>
                     Dissabled Payment
                    </label>
                  </div>
                  <!-- /.form-group -->                  
				 ...
				 <div class="col-md-6">
                   <div class="box-footer">
                    <button type="submit" name="receive_otheran_charges" class="btn btn-block btn-primary btn-flat">Submit</button>
                  </div>                 
                </div><!-- /.col -->
				</form>
              </div><!-- /.row -->
    ...
    <script>
      $(function () {
		 ...
		 $(document).on('change','.particu',function(){
			   var rwid=$(this).val();
			   var id=btoa(rwid);    			
			   var rwclid=$("#classId").val();			  
			   var clid=btoa(rwclid);
			   var rwSecId=$("#sectionId").val();			  
			   var sec=btoa(rwSecId);
			   var rwYear=$("#yr").val();			  
			   var year=btoa(rwYear);   
			    $.ajax({
				  url:'ajax_other_anulchr_fee',
				  data:{id:id,},
				  type : 'POST' ,
				  cache:false,
				  success:function(data){
				  $(".st_otan_chr").html(data);			
				 } 
		      }); 
               $.ajax({
				  url:'ajax_recv_otran_chrgs',
				  data:{id:id},
				  type : 'POST' ,
				  cache:false,
				  success:function(data){
				  $(".recv_otan_chrgs").html(data);			
				 } 
		       });
              	$.ajax({
				  url:'ajax_roll_num_recv_otranuch',
				  data:{id:id,cls:clid,sec:sec,year:year},
				  type : 'POST' ,
				  cache:false,
				  success:function(data){
